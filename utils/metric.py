...
# nIou
def batch_pix_accuracy_n(output, target):
    if len(target.shape) == 3:
...
def batch_intersection_union_n(output, target, nclass):
    predict = (output > 0).float()
    if len(target.shape) == 3:
        target = torch.unsqueeze(target.float(), dim=1)
    elif len(target.shape) == 4:
    ...
    intersection = np.zeros((nclass,), dtype=np.float32)
    union = np.zeros((nclass,), dtype=np.float32)

    for i in range(nclass):
        intersection[i] = torch.logical_and(predict == i, target == i).sum().cpu().numpy()
        union[i] = torch.logical_or(predict == i, target == i).sum().cpu().numpy()

    assert (intersection <= union).all(), "Error: Intersection area should be smaller than Union area"
