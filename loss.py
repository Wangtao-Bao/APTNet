...
class SLSIoULoss(nn.Module):
    ...
    def forward(self, pred_log, target, warm_epoch, epoch, with_shape=True):
        ...
        # 首先对预测的 logits 张量进行 sigmoid 激活，得到概率值 pred,因为 Ground Truth图像的像素值是 0-1的
...
# 带有像素损失
class SLSIoUPixelLoss(nn.Module):
    def __init__(self):
        super(SLSIoUPixelLoss, self).__init__()
    ...
    def forward(self, pred_log, target, warm_epoch, epoch, with_shape=True):
        ...
        dis = torch.pow((pred_sum - target_sum) / 2, 2)
        alpha = (torch.min(pred_sum, target_sum) + dis + smooth) / (torch.max(pred_sum, target_sum) + dis + smooth)
        iou_loss = (intersection_sum + smooth) / (pred_sum + target_sum - intersection_sum + smooth)

        # 计算形状感知损失
        lloss = LLoss(pred, target)

        # 计算像素损失 (L1损失)
        # pixel_loss = F.l1_loss(pred, target, reduction='mean')
        # # 计算像素损失 (L2损失)
        pixel_loss = F.mse_loss(pred, target, reduction='mean')

        if epoch > warm_epoch:
            siou_loss = alpha * iou_loss
            if with_shape:
                loss = 1 - siou_loss.mean() + lloss + pixel_loss_weight * pixel_loss
            else:
                loss = 1 - siou_loss.mean() + pixel_loss_weight * pixel_loss
        else:
            loss = 1 - iou_loss.mean() + pixel_loss_weight * pixel_loss
